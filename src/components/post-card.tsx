...
const PostCardComponent = ({ post: initialPost }: PostCardProps) => {
      ...
      const response = await fetch(`/api/posts/${post.id}`,
        {
          ...
          body: JSON.stringify({ userId: userProfile.id, action: 'like' })
        });
      ...
      const response = await fetch(`/api/posts/${post.id}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: userProfile.id, action: 'bookmark' })
        });
  ...
  const handleTip = useCallback(async () => {
    if (isGuest || !userProfile || isProcessing || isOwner) {
      if (isGuest) showLoginToast();
      if (isOwner) toast({ title: "You cannot tip your own post.", variant: "destructive" });
      return;
    }

    setIsTipping(true);
    try {
      await processTip(userProfile.id, post.user.id, tipAmount);
      toast({ title: "Tip sent successfully!", description: `You sent $${tipAmount / 100} to ${post.user.username}.` });
  ...
  }, [isGuest, userProfile, isProcessing, isOwner, post.user.id, post.user.username, tipAmount, showLoginToast, toast]);
  ...
  const handleSponsor = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/posts/${post.id}/sponsor`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: userProfile.id })
      ...
      const updatedPost = await response.json();
      setPost(updatedPost);
      toast({ title: "Post sponsored successfully!" });
      setIsSponsorDialogOpen(false);
    } catch (error) {
      console.error("Error sponsoring post:", error);
      toast({ title: "Failed to sponsor post", variant: "destructive" });
    } finally {
      setIsSponsoring(false);
    }
  }, [isOwner, post.id, userProfile, toast]);
  ...
  return (
    <>
      <Card className={cn("overflow-hidden animate-fade-in bg-transparent border-0 border-b rounded-none shadow-none", isProcessing && "opacity-50 pointer-events-none")}>
        <CardHeader className="flex flex-row items-center gap-3 p-4">
          ...
          <div className="grid gap-0.5 text-sm">
            ...
            <div className="text-muted-foreground">
              ...
              {post.isSponsored && <span className="ml-2 font-semibold text-primary">&bull; Sponsored</span>}
            </div>
          </div>
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              {isOwner && !post.isSponsored && (
                <>
                  <DropdownMenuItem onSelect={() => setIsSponsorDialogOpen(true)}>
                    <Icons.sponsor className="mr-2 h-4 w-4" />
                    Sponsor Post
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
                </>
        ...
        <CardFooter className="p-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-1">
            ...
            <Button variant="ghost" size="sm" onClick={() => setIsTipDialogOpen(true)} aria-label="Tip post" className="flex items-center gap-2" disabled={isProcessing || isOwner}>
              <Icons.dollar className="text-foreground/70 h-5 w-5" />
            </Button>
          </div>
      ...
      <Dialog open={isTipDialogOpen} onOpenChange={setIsTipDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Send a Tip to @{post.user.username}</DialogTitle>
          </DialogHeader>
          <div className="py-4 space-y-4">
            <p className="text-muted-foreground">Show your appreciation for this post by sending a tip.</p>
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={handleTip} disabled={isTipping}>
              {isTipping && <Icons.spinner className="animate-spin mr-2" />}
              Send Tip
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent>
          ...
          <div className="py-4 space-y-4">
            <p className="text-muted-foreground">
              Sponsoring your post will increase its visibility on the platform. A one-time fee of $5.00 will be charged.
            </p>
          </div>
          <DialogFooter>
            <DialogClose asChild>
            ...
            <Button type="button" onClick={handleSponsor} disabled={isSponsoring}>
              {isSponsoring && <Icons.spinner className="animate-spin mr-2" />}
              Sponsor for $5.00
            </Button>
          </DialogFooter>
        </DialogContent>
